...
gxMesh* Surface::getSkinnedMesh() {
	if (skinned_mesh && skinned_valid_vs == (int)vertices.size() && skinned_valid_ts == (int)triangles.size()) return skinned_mesh;

	invalidateSkinned();

	skinned_mesh = gx_graphics->createSkinnedMesh(vertices.size(), triangles.size());
	if (!skinned_mesh) return nullptr;

	skinned_mesh->lock(true);
	...
	for (size_t i = 0; i < vertices.size(); ++i) {
		...
		if (v.bone_bones[0] == 255) {
			...
			for (int j = 0; j < MAX_SURFACE_BONES; ++j) {
				if (v.bone_bones[j] == 255) break;
				weights[j] = v.bone_weights[j];
				indices[j] = v.bone_bones[j];
				boneCnt++;
			}
		}
		skinned_mesh->setSkinVertex(i, (float*)&v.coords, (float*)&v.normal, v.color, v.tex_coords, weights, indices);
	}

	for (size_t i = 0; i < triangles.size(); ++i) {
		skinned_mesh->setTriangle(i, triangles[i].verts[0], triangles[i].verts[1], triangles[i].verts[2]);
	}

	skinned_mesh->unlock();
	skinned_valid_vs = vertices.size();
	skinned_valid_ts = triangles.size();
	return skinned_mesh;
}

void Surface::invalidateSkinned() {
	if (skinned_mesh) {
		gx_graphics->freeMesh(skinned_mesh);
		skinned_mesh = nullptr;
	}
	skinned_valid_vs = skinned_valid_ts = 0;
}